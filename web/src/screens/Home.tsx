...
const Analytics = lazy(() =>
	import("@/components/stats/Analytics").then((m) => ({
		default: m.Analytics,
	})),
);
...
const FEATURES = [
	{
		...
		description:
			"Round-robin, weighted, performance-based, geo-targeted, and more.",
	},
	{
		icon: Zap,
		title: "Async-First Design",
		description:
			"Built for high-throughput with httpx and native async support.",
	},
	{
		icon: Shield,
		title: "Circuit Breakers",
		description:
			"Automatic proxy ejection and recovery with smart health checks.",
	},
	{
		icon: Server,
		title: "REST API & CLI",
		description:
			"Multiple interfaces for any integration or automation pipeline.",
	},
];
...
const CODE_EXAMPLES = [
	{
		...
		label: "Python",
		filename: "main.py",
		code: `from proxywhirl import ProxyWhirl

# The empty pool is hydrated from enabled built-in sources on first use.
rotator = ProxyWhirl()
...
async def main():
    async with AsyncProxyWhirl() as rotator:
        tasks = [
            rotator.get(f"https://api.example.com/{i}")
            for i in range(100)
        ]
...
$ uvx proxywhirl pool list --format json

# Route requests through rotating proxies
$ uvx proxywhirl request https://httpbin.org/ip

# View pool health and performance stats
$ uvx proxywhirl stats`,
	},
];
	...
	const handleCopyAll = useCallback(async () => {
		...
		const filteredProxies = proxyData.proxies.filter((p) => {
			...
			if (
				...
				(!p.country_code || !filters.countries.includes(p.country_code))
			)
				return false;
			return true;
		});
		const proxyList = filteredProxies
			.map((p) => `${p.ip}:${p.port}`)
			.join("\n");
			...
			<div className="space-y-16">
				...
				>
					...
					<motion.p
						...
						variants={slideUp}
					>
						Production-grade proxy rotation for Python. Auto-fetching,
						validation, circuit breakers, and 9 rotation strategies out of the
						box.
					</motion.p>
					...
					<motion.div className="max-w-md mx-auto" variants={slideUp}>
						<div className="flex items-center gap-2 rounded-lg border bg-muted/30 px-4 py-3 font-mono text-sm">
							...
							<code className="flex-1 text-left">uv add proxywhirl</code>
							<button
								onClick={async () => {
									const success = await copyToClipboard("uv add proxywhirl");
					...
					<section className="space-y-4">
									...
									>
										<div>
											<p className="font-semibold">
												{PROTOCOL_LABELS[protocol as Protocol]}
											</p>
						...
						>
							...
							<span className="font-semibold">
								Download All ({stats.proxies.total.toLocaleString()} proxies)
							</span>
							{stats.file_sizes["all.txt"] && (
								<span className="text-sm text-muted-foreground">
									• {formatBytes(stats.file_sizes["all.txt"])}
								</span>
							)}
						</a>
					</section>
				...
				<section id="proxy-table" className="space-y-4">
					<div>
						<h2 className="text-2xl font-bold tracking-tight">
							Browse & Export Proxies
						</h2>
						<p className="text-muted-foreground">
							Search, filter by protocol, and export your selection as TXT, CSV,
							or JSON.
						</p>
					</div>
					<RichProxyTable
				...
				{/* Code Examples — SDK + CLI */}
				<section className="space-y-6">
					<h2 className="text-2xl font-bold tracking-tight text-center">
						Get Started in Seconds
					</h2>
					<div className="max-w-2xl mx-auto">
						<div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-950 overflow-hidden shadow-xl dark:shadow-2xl dark:shadow-purple-500/5">
							{/* Tab bar with macOS chrome */}
							<div className="flex items-center gap-2 px-4 py-2.5 border-b border-zinc-800 bg-zinc-900/80">
								...
								<button
									...
									className="text-zinc-500 hover:text-zinc-300 transition-colors ml-1 p-1 rounded hover:bg-zinc-800/60"
									title="Copy to clipboard"
								>
									<svg
							...
							<pre className="p-5 overflow-x-auto text-[13px] leading-relaxed min-h-[200px]">
