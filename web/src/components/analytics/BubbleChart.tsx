...
export function BubbleChart({ proxies = [], stats }: BubbleChartProps) {
  const data = useMemo(() => {
    const precomputed = stats?.aggregations?.by_country_detail
    if (precomputed && precomputed.length > 0) {
      return precomputed
        .map((entry) => {
          const continent =
            entry.continent_code || COUNTRY_TO_CONTINENT[entry.code] || "AS"
          const avgResponse = entry.avg_response_ms ?? 0
          return {
            country: entry.code,
    ...
    const countryData: Record<
      string,
      { count: number; totalResponse: number; responseSamples: number }
    > = {}
    ...
    return Object.entries(countryData)
      .map(([code, countryStats]) => {
        ...
        return {
          ...
          avgResponse:
            countryStats.responseSamples > 0
              ? Math.round(countryStats.totalResponse / countryStats.responseSamples)
              : 0,
          continent,
          continentName: CONTINENT_LABELS[continent] || continent,
          color: CONTINENT_COLORS[continent] || "#6b7280",
        ...
        <div className="h-[400px]">
          <ResponsiveContainer width="100%" height="100%">
            <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              ...
              />
              <ZAxis type="number" dataKey="count" range={[50, 400]} name="Count" />
              <Tooltip
