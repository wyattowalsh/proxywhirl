...
$ proxywhirl pool list --format json
        ...
        <div className="max-w-2xl mx-auto">
          <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-950 overflow-hidden shadow-xl dark:shadow-2xl dark:shadow-purple-500/5">
            ...
            <div className="flex items-center gap-2 px-4 py-2.5 border-b border-zinc-800 bg-zinc-900/80">
              ...
              <div className="flex-1" />
              <span className="text-[11px] text-zinc-600 hidden sm:inline">{CODE_EXAMPLES[codeTab].filename}</span>
              <button
                onClick={async () => {
                  const success = await copyToClipboard(CODE_EXAMPLES[codeTab].code)
                  if (success) toast.success("Copied to clipboard")
                }}
            ...
            <pre className="p-5 overflow-x-auto text-[13px] leading-relaxed min-h-[200px]">
              <code className="text-zinc-200 font-mono">{CODE_EXAMPLES[codeTab].code}</code>
            </pre>
          </div>
        </div>
