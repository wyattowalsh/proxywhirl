...
Production requests for `/proxy-lists/*` are served from `docs/proxy-lists/` on the `main` branch of GitHub, so the scheduled list update does not need a Vercel rebuild. The Ignored Build Step skips a production build when a push only changes `web/public/proxy-lists/`.
