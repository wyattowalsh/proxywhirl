...
Write actions are rejected without a valid API key unless
`PROXYWHIRL_MCP_ALLOW_UNAUTHENTICATED_WRITES=true` is explicitly set for local
...
| Concern | Guidance |
| ------- | -------- |
| Authentication | Set `PROXYWHIRL_MCP_API_KEY` before exposing the MCP server or read resources outside localhost |
