---
title: "004: Storage Backend Decisions"
...
- **WAL vs DELETE journal**: Portability vs write performance
...
**Journal Mode**: DELETE (default) for single-file portability
- Avoids `-wal`/`-shm` sidecar files when copying or archiving the database
- Runtime pool default: `./proxywhirl.db` (gitignored; populated by `proxywhirl fetch`)
...
6. **CI Export Workflow**:
   - CI builds a fresh `proxywhirl.db` during fetch/validate, then exports lists
   - Committed artifacts: `docs/proxy-lists/` and mirrored web assets (not the DB)
   - Community access via exported text/JSON lists and the docs site
...
### CI Proxy List Export

**CI/CD Pipeline** (`.github/workflows/generate-proxies.yml`):
```yaml
- name: Fetch proxies
  run: uv run proxywhirl fetch --timeout 10 --concurrency 2000
...
- name: Commit lists
  run: |
    git add docs/proxy-lists/ web/public/proxy-lists/
...
**Benefits**:
- Community access to up-to-date proxy lists without bloating the repo
- No external API required for published list downloads
- Reproducible exports from the same fetch pipeline
