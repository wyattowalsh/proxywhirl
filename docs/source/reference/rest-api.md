...
|----------|---------|----------|-------------|
| `round-robin` | `round_robin` | Fair distribution | Perfect load balance |
| `random` | — | Unpredictable patterns | Uniform distribution |
| `weighted` | — | Prefer best proxies | Success-rate based |
| `least-used` | `least_used` | Even load balance | Tracks request counts |
| `performance-based` | `performance_based` | Fastest healthy proxies | EMA latency tracking |
| `session-persistence` | `session`, `session_persistence` | Sticky sessions | Session affinity |
...
|-------|------|-------------|
| `rotation_strategy` | string | Any supported strategy or alias listed above |
