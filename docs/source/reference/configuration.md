...
- **Values**: `"round-robin"`, `"random"`, `"weighted"`, `"least-used"`, `"performance-based"`, `"session-persistence"`, `"geo-targeted"`, `"cost-aware"`; underscore aliases such as `"round_robin"` and `"performance_based"` are accepted
- **Default**: `"round-robin"`
...
|-------|------|---------|------------|-------------|
| `rotation_strategy` | `str` | `"round-robin"` | Supported rotator strategy name or alias | Proxy rotation algorithm |
...
- `"performance-based"` - Select low-latency proxies using EMA performance data
- `"session"` / `"session-persistence"` - Maintain sticky proxy assignments
- `"geo-targeted"` - Select proxies matching geographic preferences
- `"cost-aware"` - Prefer free or lower-cost proxies

:::{note}
Underscore aliases are accepted for strategy names that include hyphens, for example `"round_robin"`, `"least_used"`, `"performance_based"`, `"session_persistence"`, `"geo_targeted"`, and `"cost_aware"`. `CompositeStrategy` is available in Python when passed as an instantiated strategy object. See [Python API](python-api.md) and [Advanced Strategies](../guides/advanced-strategies.md).
:::
