...
Valid strategies: `round-robin`, `random`, `weighted`, `least-used`, `performance-based`, `session`, `session-persistence`, `geo-targeted`, and `cost-aware`. Underscore aliases are accepted for hyphenated names. For detailed strategy configuration and tuning, see {doc}`advanced-strategies`.
