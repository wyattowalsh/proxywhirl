...
``"geo-targeted"``, and ``"cost-aware"``. Underscore variants are also accepted
(for example, ``"round_robin"``, ``"geo_targeted"``, and ``"cost_aware"``).
For ``composite``, pass an instantiated strategy object. You can also hot-swap
strategies at runtime with {meth}`set_strategy() <proxywhirl.ProxyWhirl.set_strategy>`.
```
