...
**A:** Python 3.10+. We test on 3.10, 3.11, 3.12, and 3.13.
...
**A:** Core: httpx, pydantic, sqlmodel, loguru, tenacity. Optional: fastapi (REST API), typer (CLI), playwright (JS rendering).
...
3. **CLI**: `proxywhirl` command for request, pool, config, fetch, import, export, and health workflows
4. **MCP**: Anthropic MCP server for AI assistants

### Q: How do I start the REST API?

**A:** `uv run uvicorn proxywhirl.api:app --host 127.0.0.1 --port 8000`
