...
class HealthCheckConfig(BaseModel):
    ...
    check_interval_seconds: int = Field(
        default=60,
        ge=10,
        description="Seconds between health checks (minimum 10)"
    )
    ...
    check_url: str = Field(
        ...
        description="Target URL to check through proxy"
    )
    ...
    max_recovery_attempts: int = Field(
        default=5,
        ge=0,
        description="Maximum recovery attempts before permanent failure"
    )
    recovery_cooldown_base_seconds: int = Field(
        default=60,
        gt=0,
        description="Base cooldown period for recovery (exponential backoff)"
    )
    source_intervals: dict[str, int] = Field(
        default_factory=dict,
        description="Per-source interval overrides (source name -> seconds)"
    )
    history_retention_hours: int = Field(
        default=24,
        ge=1,
        description="How long to keep health check history"
    )
