...
def configure_logging(level: str = "INFO"):
    """Configure comprehensive logging for the API server."""
...
def serve(
    host: str = typer.Option(
        os.getenv("PROXYWHIRL_HOST", "127.0.0.1"), 
        "--host", "-h", 
        help="Host to bind the server to"
    ),
    port: int = typer.Option(
        int(os.getenv("PROXYWHIRL_PORT", "8000")), 
        "--port", "-p", 
    ...
    """
    Start the ProxyWhirl FastAPI server with comprehensive configuration.

    The server provides a complete REST API for managing rotating proxies
    ...
    try:
        if production:
            ...
            gunicorn_config = [
                ...
                "--access-logfile", "-" if access_log else "/dev/null",
                "--error-logfile", "-",
                "--preload",
            ]
...
def health():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            logger.success("✅ ProxyWhirl API is healthy")
        ...
        logger.info("   Make sure the server is running with: python -m proxywhirl.api.server serve")
        sys.exit(1)
    except Exception as e:
