...
- [ ] T001 Verify pyproject.toml has correct dependencies (httpx>=0.25.0, pydantic>=2.0.0, pydantic-settings>=2.0.0, tenacity>=8.2.0, loguru>=0.7.0)
- [ ] T002 Verify pyproject.toml has correct dev dependencies (pytest>=7.4.0, pytest-cov>=4.1.0, hypothesis>=6.88.0, mypy>=1.5.0, ruff>=0.1.0, black>=23.0.0)
- [ ] T003 Verify pyproject.toml has correct optional dependencies ([js]: playwright>=1.40.0, [storage]: sqlmodel>=0.0.14, [security]: cryptography>=41.0.0)
- [ ] T004 Create proxywhirl/__init__.py with empty __all__ list (will populate later)
- [ ] T005 [P] Create proxywhirl/py.typed marker file for PEP 561 compliance
- [ ] T006 [P] Configure ruff.toml for linting rules
- [ ] T007 [P] Configure mypy.ini for --strict type checking
- [ ] T008 [P] Create tests/conftest.py with pytest fixtures for mock proxies and pools
...
- [ ] T009 [P] Create tests/unit/test_exceptions.py with tests for all exception types
- [ ] T010 [P] Create tests/unit/test_models.py with Pydantic validation tests for Proxy model (URL+Port deduplication, SecretStr credentials, health states)
- [ ] T011 [P] Create tests/unit/test_models.py tests for ProxyPool model (add/remove, max_pool_size, deduplication)
- [ ] T012 [P] Create tests/unit/test_utils.py with tests for credential redaction (*** in logs), URL validation, logging setup

### Implementation for Foundation

- [ ] T013 [P] Implement proxywhirl/exceptions.py with exception hierarchy (ProxyError, ProxyPoolEmptyError, ProxyValidationError, ProxyAuthError, ProxyFetchError, ProxyParsingError)
- [ ] T014 Implement proxywhirl/models.py with Proxy model (Pydantic v2, SecretStr for credentials, URL+Port deduplication logic, health states: Healthy/Unhealthy/Dead, source tracking)
- [ ] T015 Implement proxywhirl/models.py with ProxyPool model (add/remove/update methods, max_pool_size validation, thread-safe operations, statistics methods)
- [ ] T016 Implement proxywhirl/models.py with ProxyCredentials model (SecretStr for username/password, never in JSON)
- [ ] T017 Implement proxywhirl/models.py with ProxyConfiguration model (timeout settings, retry config, rate limiting config)
- [ ] T018 Implement proxywhirl/models.py with enums (HealthStatus, ProxySource, ProxyFormat, ValidationLevel)
- [ ] T019 [P] Implement proxywhirl/utils.py with URL validation helpers (parse, validate format, extract protocol)
- [ ] T020 [P] Implement proxywhirl/utils.py with credential redaction for logging (ensure *** always shown, never actual values)
- [ ] T021 [P] Implement proxywhirl/utils.py with logging setup (configurable DEBUG/INFO/WARNING/ERROR levels via loguru)
- [ ] T022 Run tests: pytest tests/unit/test_exceptions.py tests/unit/test_models.py tests/unit/test_utils.py (all should pass)
- [ ] T023 Verify mypy --strict proxywhirl/ passes with no warnings
- [ ] T024 Verify coverage for security code (credential handling) is 100%
...
- [ ] T025 [P] [US1] Create tests/unit/test_strategies.py with tests for RoundRobinStrategy (sequential selection, O(1) performance <1ms)
- [ ] T026 [P] [US1] Create tests/unit/test_strategies.py with tests for RandomStrategy (uniform distribution, O(1) performance <1ms)
- [ ] T027 [P] [US1] Create tests/property/test_rotation_properties.py with Hypothesis property-based tests for round-robin (sequential order maintained over 1000 iterations)
- [ ] T028 [P] [US1] Create tests/property/test_rotation_properties.py with Hypothesis tests for random (uniform distribution over large samples)
- [ ] T029 [P] [US1] Create tests/integration/test_basic_rotation.py for acceptance scenario 1 (pip install successful with all dependencies)
- [ ] T030 [P] [US1] Create tests/integration/test_basic_rotation.py for acceptance scenario 2 (initialize rotator with proxy list, validates and loads all proxies)
- [ ] T031 [P] [US1] Create tests/integration/test_basic_rotation.py for acceptance scenario 3 (multiple requests use different proxies automatically)
- [ ] T032 [P] [US1] Create tests/integration/test_failover.py for acceptance scenario 4 (proxy failure triggers automatic fallback <100ms)

### Implementation for User Story 1

- [ ] T033 [P] [US1] Implement proxywhirl/strategies.py with RotationStrategy Protocol (interface for pluggable strategies)
- [ ] T034 [P] [US1] Implement proxywhirl/strategies.py with RoundRobinStrategy (sequential O(1) selection, <1ms guarantee)
- [ ] T035 [P] [US1] Implement proxywhirl/strategies.py with RandomStrategy (random O(1) selection, <1ms guarantee)
- [ ] T036 [US1] Implement proxywhirl/rotator.py with ProxyRotator class (pool management, strategy injection, context manager support)
- [ ] T037 [US1] Implement proxywhirl/rotator.py with add_proxy/remove_proxy methods (thread-safe, <100ms completion, FR-015, FR-017)
- [ ] T038 [US1] Implement proxywhirl/rotator.py with get_next_proxy method (uses strategy, updates stats, <1ms selection, FR-030)
- [ ] T039 [US1] Implement proxywhirl/rotator.py with automatic failover logic (switch on proxy failure, <100ms, FR-016)
- [ ] T040 [US1] Implement proxywhirl/rotator.py with retry logic using tenacity (exponential backoff, FR-027)
- [ ] T041 [US1] Implement proxywhirl/rotator.py with empty pool handling (clear exception, FR-029)
- [ ] T042 [US1] Update proxywhirl/__init__.py to export ProxyRotator, Proxy, ProxyPool, RoundRobinStrategy, RandomStrategy
- [ ] T043 [US1] Run tests: pytest tests/unit/test_strategies.py tests/property/test_rotation_properties.py tests/integration/test_basic_rotation.py tests/integration/test_failover.py
- [ ] T044 [US1] Verify performance: Proxy selection <1ms benchmark (SC-005)
- [ ] T045 [US1] Verify performance: Failover <100ms benchmark (SC-013)
- [ ] T046 [US1] Verify performance: Runtime updates <100ms benchmark (SC-009)
...
- [ ] T047 [P] [US2] Create tests/unit/test_credentials.py with tests for credential storage (SecretStr used, never exposed)
- [ ] T048 [P] [US2] Create tests/unit/test_credentials.py with tests for credential redaction in logs (always show ***)
- [ ] T049 [P] [US2] Create tests/unit/test_credentials.py with tests for credential redaction in errors (never in exception messages)
- [ ] T050 [P] [US2] Create tests/integration/test_authentication.py for acceptance scenario 1 (credentials securely stored and applied to connections)
- [ ] T051 [P] [US2] Create tests/integration/test_authentication.py for acceptance scenario 2 (authentication headers automatically included)
- [ ] T052 [P] [US2] Create tests/integration/test_auth_rotation.py for acceptance scenario 3 (correct credentials applied per proxy during rotation)

### Implementation for User Story 2

- [ ] T053 [P] [US2] Enhance proxywhirl/models.py Proxy model to extract credentials from URL and store as SecretStr
- [ ] T054 [P] [US2] Enhance proxywhirl/utils.py to ensure credentials are redacted in all log statements (DEBUG/INFO/WARNING/ERROR)
- [ ] T055 [US2] Enhance proxywhirl/rotator.py to apply credentials to httpx client connections automatically
- [ ] T056 [US2] Enhance proxywhirl/exceptions.py to sanitize credentials from exception messages
- [ ] T057 [US2] Run tests: pytest tests/unit/test_credentials.py tests/integration/test_authentication.py tests/integration/test_auth_rotation.py
- [ ] T058 [US2] Verify security: Credentials never in logs (100% compliance, SC-016)
- [ ] T059 [US2] Verify security: Credentials never in error messages (100% compliance, FR-021)
- [ ] T060 [US2] Verify coverage: Credential handling code has 100% test coverage (SC-023)
...
- [ ] T061 [P] [US3] Create tests/unit/test_pool_lifecycle.py with tests for thread-safe add/remove operations
- [ ] T062 [P] [US3] Create tests/unit/test_pool_lifecycle.py with tests for non-blocking pool updates during active requests
- [ ] T063 [P] [US3] Create tests/integration/test_runtime_updates.py for acceptance scenario 1 (add new proxies, immediately available)
- [ ] T064 [P] [US3] Create tests/integration/test_runtime_updates.py for acceptance scenario 2 (remove proxy, no longer used for new requests)
- [ ] T065 [P] [US3] Create tests/integration/test_runtime_updates.py for acceptance scenario 3 (update proxy config, changes take effect for subsequent requests)

### Implementation for User Story 3

- [ ] T066 [P] [US3] Enhance proxywhirl/models.py ProxyPool to use threading.Lock for thread-safe operations (FR-032)
- [ ] T067 [US3] Enhance proxywhirl/rotator.py add_proxy to be non-blocking, complete <100ms (FR-017, SC-009)
- [ ] T068 [US3] Enhance proxywhirl/rotator.py remove_proxy to be non-blocking, handle active requests gracefully
- [ ] T069 [US3] Implement proxywhirl/rotator.py update_proxy method for updating credentials/timeouts at runtime
- [ ] T070 [US3] Run tests: pytest tests/unit/test_pool_lifecycle.py tests/integration/test_runtime_updates.py
- [ ] T071 [US3] Verify performance: Pool updates <100ms with 50 concurrent requests (SC-009)
- [ ] T072 [US3] Verify concurrency: 1000+ concurrent requests without degradation (SC-007)
...
- [ ] T073 [P] [US4] Create tests/unit/test_strategies.py tests for WeightedStrategy (distribution matches weights, O(log n) performance)
- [ ] T074 [P] [US4] Create tests/unit/test_strategies.py tests for LeastUsedStrategy (always selects minimum usage proxy, O(log n) performance)
- [ ] T075 [P] [US4] Create tests/property/test_rotation_properties.py Hypothesis tests for weighted (distribution matches weights over 10000 samples)
- [ ] T076 [P] [US4] Create tests/property/test_rotation_properties.py Hypothesis tests for least-used (always selects minimum usage)
- [ ] T077 [P] [US4] Create tests/integration/test_strategy_switching.py for acceptance scenario 1 (round-robin produces sequential order)
- [ ] T078 [P] [US4] Create tests/integration/test_strategy_switching.py for acceptance scenario 2 (random produces uniform distribution)
- [ ] T079 [P] [US4] Create tests/integration/test_strategy_switching.py for acceptance scenario 3 (weighted prefers higher-weighted proxies)

### Implementation for User Story 4

- [ ] T080 [P] [US4] Implement proxywhirl/strategies.py with WeightedStrategy (weighted random selection, O(log n), <1ms)
- [ ] T081 [P] [US4] Implement proxywhirl/strategies.py with LeastUsedStrategy (min-heap priority queue, O(log n), <1ms)
- [ ] T082 [US4] Implement proxywhirl/rotator.py set_strategy method for runtime strategy switching (<5ms, FR-008, SC-008)
- [ ] T083 [US4] Add weight parameter to Proxy model for weighted rotation
- [ ] T084 [US4] Update proxywhirl/__init__.py to export WeightedStrategy, LeastUsedStrategy
- [ ] T085 [US4] Run tests: pytest tests/unit/test_strategies.py tests/property/test_rotation_properties.py tests/integration/test_strategy_switching.py
- [ ] T086 [US4] Verify performance: All strategies <1ms selection time (SC-005)
- [ ] T087 [US4] Verify performance: Strategy switching <5ms (SC-008)
...
- [ ] T125 [P] [US7] Create tests/unit/test_utils_coverage.py with edge case tests (all 17 edge cases from spec.md)
...
- [ ] T157 Run full test suite: pytest tests/ --cov=proxywhirl --cov-report=html
- [ ] T158 Verify coverage ≥85% (current: 88%+, must maintain)
- [ ] T159 Verify 100% coverage for credential handling code
- [ ] T160 Run mypy --strict proxywhirl/ (must pass with no warnings)
- [ ] T161 Run ruff check proxywhirl/ (must pass with no errors)
- [ ] T162 Run black --check proxywhirl/ (must pass)
