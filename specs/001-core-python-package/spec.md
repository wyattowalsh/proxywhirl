...
- What happens when all proxies in the pool fail simultaneously?
- How does the system handle proxy URLs with invalid formats or missing components?
- What occurs when a proxy requires authentication but no credentials are provided?
- How are very slow proxies (high latency, timeouts) handled in the rotation?
- What happens when the proxy pool is empty or not initialized?
- How does the system behave when proxy credentials are incorrect or expired?
- What occurs during concurrent requests when proxies are being added/removed from the pool?
- What happens when free proxy sources are unreachable or return invalid data?
- How does the system handle rate limiting from free proxy list providers?
- What occurs when fetched proxies have duplicate URLs or conflicting configurations?
- How are dead proxies removed from auto-fetched lists during periodic refreshes?
- What happens when mixing SOCKS and HTTP proxies from different sources?
- What occurs when JavaScript rendering times out or fails to load elements?
- How does the system handle proxy sources that require JavaScript but Playwright is not installed?
- What happens when a JavaScript-rendered page changes its DOM structure?
- What happens when a user-provided custom parser raises an exception?
- How does adaptive rate limiting behave when proxies have highly variable performance?
