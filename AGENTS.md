...
- Source pages live in `web/content/docs/**/*.mdx` (guides, concepts, interfaces, reference, operations).
- Next.js route and UI source lives in `web/src/app/`, `web/src/components/`, and `web/src/screens/`.
...
- For rendered web UI, verify internal navigation uses Next `Link`, icon-only controls have accessible names, decorative icons/images are hidden from assistive tech, broad `transition-all` is avoided, reduced-motion preferences are honored, and no emoji are used as UI icons.
