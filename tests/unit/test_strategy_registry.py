...
class TestStrategyRegistry:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("cost-aware", CostAwareStrategy),
            ("cost_aware", CostAwareStrategy),
        ],
    )
    def test_resolve_builtin_strategy_aliases(self, name, expected_type):
        """Test shared built-in strategy resolver accepts documented aliases."""
        assert isinstance(resolve_builtin_strategy(name), expected_type)

    def test_resolve_builtin_strategy_rejects_unknown(self):
        """Test shared built-in strategy resolver rejects unknown names."""
        with pytest.raises(ValueError, match="Unknown strategy name: unsupported"):
            resolve_builtin_strategy("unsupported")
