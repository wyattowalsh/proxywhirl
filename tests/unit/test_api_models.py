...
class TestUpdateConfigRequestValidation:
    ...
    def test_update_config_accepts_rotator_strategy_aliases(self, strategy):
        """Test API config accepts strategy names supported by rotators."""
        request = UpdateConfigRequest(rotation_strategy=strategy)
        assert request.rotation_strategy == strategy
