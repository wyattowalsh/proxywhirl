...
class TestJSONParser:
    ...
    def test_parse_json_rejects_non_object_entries(self) -> None:
        """JSON arrays must contain proxy objects."""
        from proxywhirl.fetchers import JSONParser

        parser = JSONParser()

        with pytest.raises(ProxyValidationError, match="expected object"):
            parser.parse('[{"host": "proxy1.com", "port": 8080}, "bad-entry"]')
...
class TestGeonodeParser:
    """Test GeoNode API parser normalization."""

    def test_parse_geonode_skips_invalid_protocols_and_ports(self) -> None:
        """Test GeoNode parser only returns parseable proxy URLs."""
        ...
        json_data = json.dumps(
            {
                "data": [
                    None,
                    "bad-row",
                    {"ip": "1.2.3.4", "port": "8080", "protocols": ["http", "ftp", None]},
                    {"ip": "5.6.7.8", "port": "99999", "protocols": ["http"]},
                ]
            }
        )

        proxies = GeonodeParser().parse(json_data)

        assert proxies == [{"url": "http://1.2.3.4:8080", "protocol": "http"}]

    def test_parse_geonode_brackets_ipv6_hosts(self) -> None:
        """Test GeoNode parser normalizes IPv6 hosts into valid proxy URLs."""
        from proxywhirl.fetchers import GeonodeParser

        json_data = json.dumps(
            {"data": [{"ip": "2001:db8::1", "port": 1080, "protocols": ["socks5"]}]}
        )

        proxies = GeonodeParser().parse(json_data)

        assert proxies == [{"url": "socks5://[2001:db8::1]:1080", "protocol": "socks5"}]
...
class TestPlainTextParserEdgeCases:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "http://proxy.example.com:8080?region=us",
            "http://bad host:8080",
            "http://-bad.example:8080",
            "http://example..com:8080",
        ],
    )
    def test_parse_full_url_invalid_port_or_scheme_raises(self, text_data: str) -> None:
        """Test full proxy URLs require supported schemes and explicit valid ports."""
        from proxywhirl.fetchers import PlainTextParser
    ...
    def test_parse_exception_skipped_when_skip_invalid(self) -> None:
        ...
        # First shared parser call will fail, second will succeed
