...
class TestURLValidation:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("http://bad host:8080", False),  # invalid hostname
            ("http://-bad.example:8080", False),  # label starts with hyphen
            ("http://bad-.example:8080", False),  # label ends with hyphen
            ("http://example..com:8080", False),  # empty label
            ("http://.example.com:8080", False),  # leading dot
            (f"http://{'.'.join(['a' * 63] * 4)}:8080", False),  # host too long
            ("http://proxy.example.com:8080/path", False),  # path not allowed
            ("http://proxy.example.com:8080?region=us", False),  # query not allowed
        ...
        ids=[
            ...
            "space_in_host_invalid",
            "leading_hyphen_invalid",
            "trailing_hyphen_invalid",
            "empty_label_invalid",
            "leading_dot_invalid",
            "dns_name_too_long_invalid",
            "path_invalid",
            "query_invalid",
            "malformed_invalid",
            "empty_invalid",
...
class TestURLParsing:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "http://.example.com:8080",
            f"http://{'.'.join(['a' * 63] * 4)}:8080",
            "http://proxy.example.com:8080/path",
            "http://proxy.example.com:8080?region=us",
        ],
    )
    def test_parse_invalid_url_raises_error(self, url):
...
class TestLoggingUtilities:
    ...
    def test_redact_url_credentials_malformed_userinfo(self, url):
