...
class TestParseProxyUrl:
    ...
    def test_ipv6_url_with_port(self) -> None:
        """Test parsing IPv6 URL with port."""
        ip, port = parse_proxy_url("http://[2001:db8::1]:8080")
        assert ip == "2001:db8::1"
        assert port == 8080

    def test_url_without_port_returns_empty(self) -> None:
        """Test that URL without port is treated as invalid."""
        ip, port = parse_proxy_url("http://192.168.1.1")
        assert ip == ""
        assert port == 0

    def test_url_without_hostname(self) -> None:
        """Test URL without hostname returns an empty address."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "http://192.168.1.1:notaport",
            "http://192.168.1.1:0",
            "http://192.168.1.1:65536",
        ],
    )
    def test_invalid_url_returns_empty(self, url: str) -> None:
        """Test malformed URLs are not coerced into synthetic proxy addresses."""
        ip, port = parse_proxy_url(url)
        assert ip == ""
        assert port == 0
...
class TestGenerateRichProxies:
    ...
    async def test_invalid_proxy_urls_are_skipped(self) -> None:
        """Test rich export skips malformed proxy URLs instead of emitting :0 records."""
        mock_storage = AsyncMock()
        mock_storage.load.return_value = [
            {"url": "http://192.168.1.1", "protocol": "http"},
            {"url": "http://:8080", "protocol": "http"},
            {"url": "http://192.168.1.2:8080", "protocol": "http"},
        ]

        result = await generate_rich_proxies(mock_storage, max_age_hours=0)

        assert result["total"] == 1
        assert result["proxies"][0]["ip"] == "192.168.1.2"
