...
class TestProxyConfig:
    ...
    def test_valid_ipv6_url(self) -> None:
        """Test valid IPv6 proxy URL."""
        config = ProxyConfig(url="http://[2001:db8::1]:8080")
        assert config.url == "http://[2001:db8::1]:8080"
    ...
    def test_invalid_url_host_or_port(self, url: str) -> None:
        """Test that proxy URLs require host and valid explicit port."""
...
class TestSaveConfig:
    ...
    def test_save_to_existing_pyproject_preserves_permissions(self, tmp_path: Path) -> None:
        """Test saving project config does not force private config permissions."""
        pyproject = tmp_path / "pyproject.toml"
        ...
        assert pyproject.stat().st_mode & 0o777 == 0o644

    def test_save_regular_config_uses_private_permissions(self, tmp_path: Path) -> None:
        """Test regular config files are written with owner-only permissions."""
        config_file = tmp_path / "config.toml"
        config = CLIConfig(timeout=60, encrypt_credentials=False)
