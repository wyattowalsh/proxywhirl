...
class TestValidateSource:
    ...
    async def test_validate_source_accepts_short_valid_proxy_content(self):
        ...
        assert result.status_code == 200
        assert result.content_length == 39
        assert result.has_proxies is True
        assert result.error is None
...
class TestValidateSources:
    ...
    async def test_validate_sources_with_list(self):
        ...
        sources = [
            ProxySourceConfig(url="http://example1.com/p.txt", format="plain_text"),
    ...
    async def test_validate_sources_mixed_results(self):
        ...
        sources = [
            ProxySourceConfig(url="http://good.com/p.txt", format="plain_text"),
            ProxySourceConfig(url="http://bad.com/p.txt", format="plain_text"),
        ]
...
class TestSourceProtocolTagging:
    ...
    def test_roosterkid_sources_use_raw_proxy_lists(self):
        """RoosterKid protocol sources should avoid bannered human-readable lists."""
        assert str(GITHUB_ROOSTERKID_SOCKS4.url).endswith("/SOCKS4_RAW.txt")
        assert str(GITHUB_ROOSTERKID_SOCKS5.url).endswith("/SOCKS5_RAW.txt")
