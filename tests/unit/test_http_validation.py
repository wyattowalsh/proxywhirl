...
class TestHTTPValidation:
    ...
    async def test_http_request_success(self) -> None:
        ...
        validator = ProxyValidator(level=ValidationLevel.STANDARD, test_url=TEST_URL)
    ...
    async def test_http_request_timeout(self) -> None:
        ...
        validator = ProxyValidator(level=ValidationLevel.STANDARD, timeout=2.0, test_url=TEST_URL)
    ...
    async def test_http_request_connection_error(self) -> None:
        ...
        with respx.mock:
            respx.get(TEST_URL).mock(side_effect=httpx.ConnectError("Cannot connect"))
