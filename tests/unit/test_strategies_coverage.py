...
class TestCompositeStrategyFromConfig:
    ...
    def test_strategy_from_name_all_strategies(self) -> None:
        ...
        assert isinstance(CompositeStrategy._strategy_from_name("round_robin"), RoundRobinStrategy)
        assert isinstance(CompositeStrategy._strategy_from_name("random"), RandomStrategy)
        assert isinstance(CompositeStrategy._strategy_from_name("weighted"), WeightedStrategy)
        assert isinstance(CompositeStrategy._strategy_from_name("least-used"), LeastUsedStrategy)
        assert isinstance(CompositeStrategy._strategy_from_name("least_used"), LeastUsedStrategy)
        assert isinstance(
            CompositeStrategy._strategy_from_name("performance-based"), PerformanceBasedStrategy
        )
        assert isinstance(
            CompositeStrategy._strategy_from_name("performance_based"), PerformanceBasedStrategy
        )
        assert isinstance(
            CompositeStrategy._strategy_from_name("session"), SessionPersistenceStrategy
        )
        assert isinstance(
            CompositeStrategy._strategy_from_name("session-persistence"),
            SessionPersistenceStrategy,
        )
        assert isinstance(
        ...
        assert isinstance(
            CompositeStrategy._strategy_from_name("geo_targeted"), GeoTargetedStrategy
        )
        assert isinstance(CompositeStrategy._strategy_from_name("cost-aware"), CostAwareStrategy)
        assert isinstance(CompositeStrategy._strategy_from_name("cost_aware"), CostAwareStrategy)
