...
class TestRedactionHelpers:
    ...
    def test_redact_url_credentials_parse_error(self, monkeypatch) -> None:
        ...
        assert utils_mod._redact_url_credentials("http://user:pass@host") == "http://***:***@host"
...
class TestValidationEdgeCasesCoverage:
    ...
    def test_validate_target_url_safe_rejects_any_private_dns_result(self, monkeypatch) -> None:
        import socket
        ...
        monkeypatch.setattr(utils_mod.socket, "getaddrinfo", _getaddrinfo)

        with pytest.raises(ValueError, match="loopback"):
            utils_mod.validate_target_url_safe("https://example.com")

    def test_validate_target_url_safe_rejects_unspecified_before_private(self, monkeypatch) -> None:
        import socket
    ...
    def test_validate_target_url_safe_rejects_ipv4_mapped_loopback(self, monkeypatch) -> None:
        import socket

        from proxywhirl import utils as utils_mod

        def _getaddrinfo(*_args, **_kwargs):
            return [(socket.AF_INET6, socket.SOCK_STREAM, 0, "", ("::ffff:127.0.0.1", 0))]
