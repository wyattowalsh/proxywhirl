...
class TestProxyValidation:
    ...
    # -------------------------------------------------------------------------
    # Full URL format tests (shared parser parity)
    # -------------------------------------------------------------------------
    @pytest.mark.parametrize(
        "url",
        [
    ...
    def test_invalid_proxy_url_format_rejects(self, url):
        """Test that Proxy rejects URL shapes rejected by the shared parser."""
        with pytest.raises(ValidationError, match="Invalid proxy URL"):
            Proxy(url=url, allow_local=True)

    def test_ipv6_proxy_url_format_accepts(self):
        """Test that shared parser parity keeps bracketed IPv6 proxy URLs valid."""
