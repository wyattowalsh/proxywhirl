...
class TestProxyPoolHealthStatus:
    ...
    def test_unhealthy_count(self):
        ...
        for i in range(2):
            proxy = Proxy(
                url=f"http://unhealthy{i}.example.com:8080", health_status=HealthStatus.UNHEALTHY
            )
            pool.add_proxy(proxy)
    ...
    def test_mixed_health_status(self):
        ...
        proxy_healthy = Proxy(
            url="http://healthy.example.com:8080", health_status=HealthStatus.HEALTHY
        )
...
class TestProxyPoolStatistics:
    ...
    def test_total_requests_sum(self):
        ...
        for i in range(3):
            proxy = Proxy(url=f"http://proxy{i}.example.com:8080", total_requests=100 + (i * 10))
            pool.add_proxy(proxy)
    ...
    def test_overall_success_rate(self):
        ...
        proxy1 = Proxy(url="http://proxy1.example.com:8080", total_requests=100, total_successes=80)
        pool.add_proxy(proxy1)

        # Proxy 2: 70 successes out of 100 requests
        proxy2 = Proxy(url="http://proxy2.example.com:8080", total_requests=100, total_successes=70)
    ...
    def test_overall_success_rate_no_requests(self):
        ...
        proxy = Proxy(url="http://proxy.example.com:8080", total_requests=0, total_successes=0)
        pool.add_proxy(proxy)
