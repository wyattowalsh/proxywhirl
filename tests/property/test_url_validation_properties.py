...
class TestURLFormatVariations:
    ...
    def test_url_with_path(self, scheme: str, host: str, port: int):
        """Property: Proxy URLs with paths should be rejected."""
...
class TestEdgeCases:
    ...
    @given(valid_schemes(), public_hostnames())
    def test_urls_without_explicit_port_reject(self, scheme: str, host: str):
        """Property: Proxy URLs without explicit ports should be rejected."""
        url = f"{scheme}://{host}"
        with pytest.raises(ValidationError, match="Invalid proxy URL"):
            Proxy(url=url)
